...
def main():
    ...
    if story_file.exists():
        ...
        title = title_parts[0][:100] if title_parts else "Historia de las mujeres antiguas"
    else:
        title = f"Historia de las mujeres antiguas - {datetime.date.today()}"

    # Platform-specific content
    descriptions = {
        'youtube': f"{story[:150] if len(story) > 150 else story} #HistoriaMujeres #HistoriaAntigua #Historia #Educación",
        'instagram': f"{story[:2200] if len(story) > 2200 else story}\n\n#HistoriaMujeres #HistoriaAntigua #Historia #Educación #Shorts #Reels",
        'tiktok': f"{story[:2200] if len(story) > 2200 else story} #HistoriaMujeres #HistoriaAntigua #Historia #Educación #FYP",
        'facebook': f"{story[:63206] if len(story) > 63206 else story}\n\n#HistoriaMujeres #HistoriaAntigua #Historia #Educación",
        'threads': f"{story[:500] if len(story) > 500 else story} #HistoriaMujeres #HistoriaAntigua #Historia #Educación",
        'twitter': f"{story[:280] if len(story) > 280 else story} #HistoriaMujeres #HistoriaAntigua #Historia",
        'vk': f"{story[:220] if len(story) > 220 else story}\n\n#HistoriaMujeres #HistoriaAntigua #Historia #Educación"
    }

    tags = [
        'Historia', 'Mujeres antiguas', 'Hechos históricos',
        'Shorts', 'Reels', 'IA', 'Educación'
    ]
